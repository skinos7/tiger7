...
# SkinOS API简介   
SkinOS API即 **统一的接口**
- 在SkinOS中以 **组件** 的方式对外提供这些接口
- 并以 **组件** 来划分 **功能**, 每个组件对应不同的功能项   
...
以Linux为基础的SkinOS即可运行于嵌入式平台中当 **网关或路由器** 使用, 也可以运行于X86平台当 **企业级路由器或服务器及云管理平台** 使用   

# SkinOS适用的场景   
- 可运行用嵌入式平台中, 如 **网关**, 终端路由器, 各种终端智能设备
- 可运行用X86的工控机中, 如企业网关, 无人机器工控设备, 控制器等   
- 栽剪后还可运行用Ubuntu系统上, 如 **网关云管理**, 云服务器, 管理服务器, 应用服务器等   

**网关** 及 **网关云管理** 是SkinOS首先应用的产品, 接下来将介绍这两个产品   
...
以上原因导致网关产品维护复杂, 更新换代及对接外设带来大量重复工作, 因此非常合适SkinOS的生存   
在网关中运行SkinOS, 向开发者及管理者或使用者提供统一的API, 并相对传统网关具有以下优势   
...
**更多介绍见此文档 [<<云管理用户网页界面使用介绍>>](https://gitee.com/tiger7/doc/blob/master/use/cloud_tui_use_cn.md)**   

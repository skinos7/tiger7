...
//add by qingcheng, support mtd part size for dts reg
#define HIGH_BIT_SHIFT(x)	(1 << (31-x))
//mtd max size  bit31 ~ bit 23 ex:8MB 0x100000
#define MAX_MTDSIZE_SHIFT		9   
// reg is pairlike <begin size> 
	int reg_offset = MAX_MTDSIZE_SHIFT * 2;
	uint32_t tmp = (uint32_t)master->size;
	tmp -= 1;
	for (i = 0; i < MAX_MTDSIZE_SHIFT; i++) {
		if (tmp & HIGH_BIT_SHIFT(i)) {
			break;
		}
		reg_offset -= 2;
	}

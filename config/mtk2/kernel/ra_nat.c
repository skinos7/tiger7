...
//add by qingcheng
static char *gwwan0 = NULL;
module_param(gwwan0, charp, S_IRUGO);
...
void get_bridge_info(void)
{
    ...
    // modify by dimmalex 
	br0_dev = dev_get_by_name(&init_net,"lan");
...
//modify by qingcheng, Howervey FOE_SP is any value,must set vlan1 etype and vlan layer
#if defined (CONFIG_QDMA_SUPPORT_QOS)
			foe_entry->ipv4_hnapt.iblk2.fqos = 1;
#else
			foe_entry->ipv4_hnapt.iblk2.fqos = 0;
...
#define MIN_NET_DEVICE_FOR_MESH                 0x30
    ...
    // modify by dimmalex 
	else if ( gwwan0 != NULL && strncmp(skb->dev->name, gwwan0, 4 ) == 0 ) {
	    offset = DP_USB;
	}
	//else if (strncmp(skb->dev->name, "eth1", 4) == 0) {
	//    offset = DP_USB;
...
struct net_device *__ra_dev_get_by_name(const char *name)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
...
static void PpeSetDstPort(uint32_t Ebl)
{
    ...
    if ( gwwan0 != NULL )
    {
        DstPort[DP_USB] = __ra_dev_get_by_name(gwwan0);	// USB interface name
        printk("%s ifindex =%x\n",gwwan0, DP_USB);
    }
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Ralink Hardware NAT v2.60\n");

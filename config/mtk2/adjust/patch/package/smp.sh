...
# add by dimmalex for wan instead eth1
WAND=$2
if [ "X$WAND" = "X" ]; then
    echo "insufficient arguments for wan device!"
fi
...
if [ $OPTIMIZED_FOR == "wifi" ]; then
    if [ $NUM_OF_CPU == "4" ]; then
        if [ "$CONFIG_RALINK_MT7621" = "y" ]; then
            ...
            if [ -d "/sys/class/net/rai0" ]; then
                if [ "$CONFIG_FIRST_CARD" = "MT7615D" -a "$CONFIG_SECOND_CARD" = "MT7663" ]; then
                    ...
                    write_proc 9 /sys/class/net/${WAND}/queues/rx-0/rps_cpus
                    #/usr/bin/logger "eth0/${WAND} RPS: CPU0/3"
                else
                    write_proc 5 /sys/class/net/eth0/queues/rx-0/rps_cpus
                    write_proc 5 /sys/class/net/${WAND}/queues/rx-0/rps_cpus
                    #/usr/bin/logger "eth0/${WAND} RPS: CPU0/2"
                fi
            else
                if [ "$DBDC" = "1" ]; then
                    ...
                    write_proc 3 /sys/class/net/${WAND}/queues/rx-0/rps_cpus
                    #/usr/bin/logger "eth0/${WAND} RPS: CPU0/1"
                fi
            fi
        fi
        ...
        if [ "$CONFIG_RALINK_MT7621" = "y" ]; then
            ...
            write_proc 2 /sys/class/net/${WAND}/queues/rx-0/rps_cpus
        fi
    fi
elif [ $OPTIMIZED_FOR == "5g" ]; then
    if [ $NUM_OF_CPU == "4" ]; then
        if [ "$CONFIG_RALINK_MT7621" = "y" ]; then
            write_proc 2 /proc/irq/3/smp_affinity  #GMAC
            write_proc 4 /proc/irq/4/smp_affinity  #PCIe0
            ...
            if [ -d "/sys/class/net/rai0" ]; then
                if [ "$CONFIG_FIRST_CARD" = "MT7615D" -a "$CONFIG_SECOND_CARD" = "MT7663" ]; then
                    write_proc c /proc/irq/22/smp_affinity #USB
                    write_proc 9 /sys/class/net/eth0/queues/rx-0/rps_cpus
                    write_proc c /sys/class/net/${WAND}/queues/rx-0/rps_cpus
                    #/usr/bin/logger "eth0/${WAND} RPS: CPU0/3"
                else
                    write_proc c /proc/irq/22/smp_affinity #USB
                    write_proc 5 /sys/class/net/eth0/queues/rx-0/rps_cpus
    ...
    write_proc 1 /sys/class/net/${WAND}/queues/rx-0/rps_cpus
    fi
else

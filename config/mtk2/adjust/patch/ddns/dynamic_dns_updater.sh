...
# add by dimmalex for debug
write_log 7 "Service client: $SECTION_ID"
write_log 7 "Service name: $service_name"
write_log 7 "Service check: $lookup_host"
write_log 7 "Service domain: $domain"
write_log 7 "Service username: $username"
write_log 7 "Service password: $password"
write_log 7 "Service interface: $ip_interface"
write_log 7 "Service check_interval: $check_interval"
write_log 7 "Service retry_interval: $retry_interval"
write_log 7 "Service url: $update_url"
write_log 7 "Service script: $update_script"
write_log 7 "Service answer: $UPD_ANSWER"
...
# add by dimmalex
echo "{ \"state\":\"register\" }" > /tmp/.ddns.$SECTION_ID
...
# add by dimmalex
echo "{ \"state\":\"register\",\"real_ip\":\"$REGISTERED_IP\" }" > /tmp/.ddns.$SECTION_ID
...
while : ; do
	...
	# add by dimmalex
	echo "{ \"state\":\"register\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${LOCAL_IP}\" }" > /tmp/.ddns.$SECTION_ID
	...
	if [ $CURR_TIME -ge $NEXT_TIME -o "$LOCAL_IP" != "$REGISTERED_IP" ]; then
		...
		if [ $ERR_LAST -eq 0 ]; then
			...
			# add by dimmalex
			echo "{ \"state\":\"succeed\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${LOCAL_IP}\" }" > /tmp/.ddns.$SECTION_ID
			...
			# add by dimmalex
			echo "{ \"state\":\"already\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${LOCAL_IP}\" }" > /tmp/.ddns.$SECTION_ID			

		else
			write_log 3 "IP update not accepted by DDNS Provider"

			# add by dimmalex
			echo "{ \"state\":\"deny\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${LOCAL_IP}\" }" > /tmp/.ddns.$SECTION_ID			

		fi

    # add by dimmalex
    else
	echo "{ \"state\":\"noneed\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${LOCAL_IP}\" }" > /tmp/.ddns.$SECTION_ID
